...
from yearly_graph_window_ui import Ui_YearlyGraphMainWindow
from display_graph import DisplayGraphDialog


class YearlyGraphWindow(QMainWindow):

    def __init__(self, x_vals, y_vals, overall_total_income, graph_type, fig_index, from_year, to_year):
        # argument: list(int), list(float), float, str, int, str, str
        super().__init__()
        self.ui = Ui_YearlyGraphMainWindow()
        self.ui.setupUi(self)
        self.win_title = "From " + from_year + " To " + to_year
        if graph_type == "line_graph":
            self.win_title = "Annual Sales Line Graph - " + "From " + from_year + " To " + to_year
        else:
            self.win_title = "Annual Sales Bar Chart - " + "From " + from_year + " To " + to_year
