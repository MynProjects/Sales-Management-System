...
from monthly_graph_window_ui import Ui_MonthlyGraphMainWindow
from display_graph import DisplayGraphDialog


class MonthlyGraphWindow(QMainWindow):

    def __init__(self, x_vals, y_vals, overall_total_income, graph_type, fig_index, year):
        # argument: list(int), list(float), float, str, int, str
        super().__init__()
        self.ui = Ui_MonthlyGraphMainWindow()
        self.ui.setupUi(self)
        self.win_title = year
        if graph_type == "line_graph":
            self.win_title = "Monthly Sales Line Graph - " + year
        else:
            self.win_title = "Monthly Sales Bar Chart - " + year
